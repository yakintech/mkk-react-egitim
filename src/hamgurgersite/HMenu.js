...
function HMenu() {
  ...
  const foods = [
    ...
    {
      name: 'Honey Almond Granola with Fruit',
    ...
    {
      name: 'Adanan Kebap',
      description: 'Acılı kebap içeriği. lorem ipsum',
      price: 17.50
    },
    {
      name: 'Belgian Waffle3',
      description: 'Vanilla flavored batter with malted flour',
      price: 27.50
    }
...

          {
            foods.map((item) => {
              return (
                <>
                  <h4>{item.name}</h4>
                  <p className="w3-text-grey">{item.description} {item.price.toFixed(2)}</p><br />
                </>)
            })
          }
